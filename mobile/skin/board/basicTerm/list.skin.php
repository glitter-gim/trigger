        ...
        <div class="list_01">
            ...
            <ul>
                <?php for ($i=0; $i<count($list); $i++) {
                    // glitter 🔽 만료일(wr_7)이 지정되어 있고, 현재 날짜보다 이전인 경우
                    if (!empty($list[$i]['wr_7']) && strtotime($list[$i]['wr_7']) < strtotime(date('Y-m-d'))) {
                        continue;
                    }   // -- glitter 🔼 해당 게시물은 건너뛰어 렌더링하지 않음

                    // glitter 🔽만료 임박 여부 확인: 만료일이 '0000-00-00' 또는 빈 값이 아니고,
                    $expireSoon = "";
                    if (!empty($list[$i]['wr_7']) && $list[$i]['wr_7'] != '0000-00-00'
                        && strtotime($list[$i]['wr_7']) <= strtotime(date('Y-m-d', strtotime('+3 days')))) {
                        $expireSoon = "tr_delete_soon";
                    }  // -- glitter 🔼 현재 날짜 기준 3일 이내면 경고 스타일 적용
                    ?>
                    ...
                    <li class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
                        ...
                        <div class="bo_info">
                            ...
                            <span class="bo_date"><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $list[$i]['datetime2'] ?></span> <!-- glitter 🔽 -->
                            <span class="td_datetime delete_date_col <?php echo !empty($list[$i]['wr_7']) ? 'has_date' : ''; ?>">
