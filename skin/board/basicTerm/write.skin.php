...
<section id="bo_w">
    ...
    <div class="bo_w_tit write_div bo_w_options">
        <!-- 옵션 체크박스 -->
        <?php if ($option) { ?>
        ...
        <div class="delete_date_wrapper"> <!-- glitter 🔽 삭제 예정 날짜 필드 추가 (옵션과 같은 줄에 정렬) -->
            <label for="delete_date"><b>해당글 <span style="color: brown;">게시 만료</span> 날짜 선택,</b></label>
            <input type="date" name="wr_7" id="delete_date" class="frm_input date_input_45"
                value="<?= isset($write['wr_7']) ? $write['wr_7'] : '' ?>" placeholder="YYYY-MM-DD">
            <label for="delete_date"><b>선택하지 않으면 <span style="color: brown;">영구 게시</span> 됨.</b></label>
        </div> <!-- glitter 🔼 삭제 예정 날짜 필드 추가 -->
    </div>
    ...
    function validateDeleteDate() {  //-- glitter 🔽 삭제 예정 날짜 필드 추가 --
        ...
        if (deleteDate) {
            ...
            if (inputDate < today) {
                alert("삭제 예정 날짜는 오늘 이후여야 합니다.");
                return false;
            }
        }
        return true;
    }
    </script> <!-- glitter 🔼 삭제 예정 날짜 필드 추가 -->
